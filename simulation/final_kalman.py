...
plt.show() # <--- CLOSE THIS WINDOW TO SEE THE NEXT ONE
...
plt.figure(figsize=(12, 10)) # Increased height for 3 plots
plt.suptitle("WINDOW 2: What if we ONLY use the Accelerometer?", fontsize=16)

# Subplot 1: Altitude Drift
plt.subplot(3, 1, 1)
...
plt.title("Altitude Drift: Integration Error Accumulates (d = 0.5*a*t^2)")
plt.ylabel('Altitude (m)')
plt.legend()
plt.grid(True)

# Subplot 2: Velocity Drift (NEW)
plt.subplot(3, 1, 2)
plt.plot(t_data, true_vel_data, 'k--', label='True Velocity', alpha=0.6)
plt.plot(t_data, accel_only_vel_data, 'c-', linewidth=2, label='Accel Only Velocity')
plt.axhline(0, color='red', linestyle=':', alpha=0.3)
plt.title("Velocity Drift: Even small noise causes velocity to wander (v = a*t)")
plt.ylabel('Velocity (m/s)')
plt.legend()
plt.grid(True)

# Subplot 3: Raw Accelerometer Data
plt.subplot(3, 1, 3)
